...
Beyond that: no comment that restates the line beneath it, no narration of your reasoning or of what you are about to do, no commented-out code, no `TODO`s — delete dead code instead. A comment earns its place only when it records something the code cannot express: a non-obvious constraint, a workaround for a specific bug, or why an odd-looking line is correct. Even then, one line usually says it. A paragraph almost never does, and a JSDoc block that needs a blank line in the middle is an essay the code should have made unnecessary.

**One line means one line.** The most common way this rule gets broken is the two-to-four line block that argues the change's case — what the code protects against, what the alternative was, the scenario that motivated it. That is the same violation in a bigger box, and it is the one agents reach for most: a block that argues for the code is narration, however true it is. Finding yourself writing one is a signal that either the code needs another name, or the reasoning belongs in the commit message. Delete the block rather than shortening it. When you finish a change, reread your own comments and delete the ones that survive only because you wrote them on the way past.
...
**Data access.** Go through the service, never around it. `BaseService` is the only thing that queries a model: use the inherited `findMany`, `findOne`, `createOne`, `createMany`, `updateById`, `updateMany`, `deleteMany` inside a service, and the sibling service from anywhere else (`playerService.findMany(...)`, `gameService.count(...)`). A bare `prisma.<model>.<operation>()` is wrong even when it works — it skips whatever query options, hooks and policies the model carries, so the same table ends up with two sets of rules. If the method you want is not on `BaseService` (there is no `upsert` or `findFirst`), read with `findOne` and branch to `updateById` or `createOne` rather than dropping to Prisma. The only exception is a standalone script, which cannot boot Arkos and has no service to call.
